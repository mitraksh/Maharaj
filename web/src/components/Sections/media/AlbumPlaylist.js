...
function AlbumPlaylist() {
    const [tracks] = useState([
      ...
      const handleClickPrevious = () => {
        setMusic((prevState) => {
          return( prevState === 0 ? tracks.length - 1 : prevState - 1)
        });
      }
      const handleClickNext = () => {
        setMusic((prevState) => {
           return(prevState < tracks.length - 1 ? prevState + 1 : 0)
        })
      }
      const nowPlayingHeader = () => {
        return(
          <div>
          <img src={tracks[music].album_art} className='album_art'/>
          {/* <Image src={tracks[music].album_art} size='small' rounded className='album_art'/> */}
          <List>
          <ListItem>
            <ListContent>
              <ListHeader as='h3'>{tracks[music].title}</ListHeader>
              <ListDescription>
                <a>
                  <b>{tracks[music].desc}</b>
                </a>{' '}
              </ListDescription>
            </ListContent>
          </ListItem>
  ...
  <GridRow stretched>
    ...
    <div>
      ...
      {tracks && tracks.map((track, index) => 
        <AudioList key={index} setList={track} playTrack={playfromList} trackid={index} />
        )}
      </List>
